...
export default function ChantsPage() {
  ...
  const getSimpleAudio = (item: any) => {
    if (!item) return null;
    // On essaie bêtement tout ce qui est possible
    return item.audio || item.url || item.audioUrl;
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-paragraph">
      ...
      {/* Selected Chant Modal - AVEC OUTIL DE DIAGNOSTIC RAYONS X */}
      <AnimatePresence>
        {selectedChant && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              {/* ========== ZONE DE DIAGNOSTIC ROUGE ========== */}
              {/* Copiez-collez ce qui apparait ici si ça ne marche pas */}
              <div className="bg-red-900/50 border border-red-500 p-4 mb-4 rounded text-xs font-mono text-red-100 break-all">
                <strong>DEBUG DATA (Envoyez moi ça):</strong> <br/>
                {JSON.stringify(selectedChant, null, 2)}
              </div>
              {/* ============================================= */}
              ...
              <div className="border-t border-white border-opacity-10 pt-8 mb-8">
                ...
                {/* On essaie le lecteur normal, sans logique compliquée */}
                {getSimpleAudio(selectedChant) ? (
                  <ModernAudioPlayer 
                    audioUrl={getSimpleAudio(selectedChant)} 
                    title={selectedChant.chantTitle}
                  />
                ) : (
                  <div className="p-4 bg-stone-900/50 border border-stone-700 rounded text-center">
                    <p className="text-sm text-stone-400">No audio detected</p>
                  </div>
                )}
              </div>
